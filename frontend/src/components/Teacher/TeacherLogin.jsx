...
function TeacherLogin() {
  ...
  const loginTeacher = async ()=>{
      setError("");
      if( !email || !password){
        setError("All details are required to register !")
      }
      try {
         const user = await axios.post(
           `${import.meta.env.VITE_BACKEND_URL}/teacher/loginTeacher`,
          {
            password : password,
            email: email
          },
    ...
    <div className="flex h-screen w-full">
      <div className="h-screen w-1/2">
        <img
          ...
          src="https://images.pexels.com/photos/33485106/pexels-photo-33485106.jpeg"
          alt="assessment demo image"
        />
      </div>

      <div class="flex min-h-screen items-center justify-center w-1/2">
        <div class="relative flex flex-col rounded-xl bg-transparent bg-clip-border text-gray-700 shadow-none">
          ...
          {error.length>0 && <div className="text-2xl text-red-500">{error}</div>} 
              ...
              >
                <p class="flex items-center font-sans text-sm font-normal leading-normal text-gray-700 antialiased">
                 Didn't have an account ? 
                  ...
                  <Link to="/teacher/registerTeacher" class="font-semibold transition-colors hover:text-pink-900 text-pink-600">
                    Sign Up Here
                  </Link>
